...
export function validateCouponForCart(
  ...
  const now = new Date();
  if (coupon.validFrom && now < new Date(coupon.validFrom)) return { ok: false, message: "Coupon not yet valid" };
  if (coupon.validUntil && now > new Date(coupon.validUntil)) return { ok: false, message: "Coupon expired" };
  ...
  const catIds = Array.isArray(coupon.applicableCategoryIds) ? coupon.applicableCategoryIds : [];
  const prodIds = Array.isArray(coupon.applicableProductIds) ? coupon.applicableProductIds : [];
